...
using SCPointType =
    IESKFSlam::Point; // using xyz only. but a user can exchange the original bin encoding function (i.e.,
                      // max hegiht) to max intensity (for detail, refer 20 ICRA Intensity Scan Context)
...
class SCManager
{
  ...
  public:
    ...
    // const double SC_DIST_THRES = 0.13; // empirically 0.1-0.2 is fine (rare false-alarms) for 20x60 polar context
    // (but for 0.15 <, DCS or ICP fit score check (e.g., in LeGO-LOAM) should be required for robustness)
    const double SC_DIST_THRES = 0.2; // 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy, DCS) + icp
                                      // fitness threshold / if not, recommend 0.1-0.15
    // const double SC_DIST_THRES = 0.7; // 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy, DCS) +
    // icp fitness threshold / if not, recommend 0.1-0.15

    // config
    const int TREE_MAKING_PERIOD_ =
        10; // i.e., remaking tree frequency, to avoid non-mandatory every remaking, to save time cost / in the
            // LeGO-LOAM integration, it is synchronized with the loop detection callback (which is 1Hz) so it means the
            // tree is updated evrey 10 sec. But you can use the smaller value because it is enough fast ~ 5-50ms wrt N.
